...
def _load_glass_file(num, pl_params):
...
def populate_particles(high_res_region, low_res_region, pl_params):
    # Initiate arrays.
    ...
    if pl_params.grid_also_glass:
        for this_glass_no in high_res_region.cell_info["num_particles_per_cell"]:
            if this_glass_no not in glass.keys():
                glass[this_glass_no] = _load_glass_file(this_glass_no, pl_params)
    else:
        glass[pl_params.glass_num] = _load_glass_file(pl_params.glass_num, pl_params)
    ...
    if mympi.comm_size > 1:
        max_lr = mympi.comm.allreduce(max_lr, op=MPI.MAX)
        min_lr = mympi.comm.allreduce(min_lr, op=MPI.MIN)
        num_lr = mympi.comm.allreduce(num_lr)
