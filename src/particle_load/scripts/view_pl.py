...
class CheckPL:
    ...
    def plot_pl(self, data):
        ...
        for i, m in enumerate(masses):
            if i > 3: break
            mask = np.where((data['coords_z'] >= data['load_coords'][2] - data['radius']*100) &
                            (data['coords_z'] <= data['load_coords'][2] + data['radius']*100) &
                            (data['masses'] == m))
            axarr[0].scatter(data['coords_x'][mask], data['coords_y'][mask], s=1)

            for j, (X,Y,xi,yi,Z,zi) in enumerate(zip(['coords_x','coords_y','coords_x'],
                    ['coords_y', 'coords_z', 'coords_z'], [0,1,0], [1,2,2],
                    ['coords_z', 'coords_x', 'coords_y'], [2,0,1])):
                mask = np.where(
                        (data[Z] >= data['load_coords'][zi] - data['cell_length']/5.) &
                        (data[Z] <= data['load_coords'][zi] + data['cell_length']/5.) &
                        (data['masses'] == m))
                axarr[1+j].scatter(data[X][mask], data[Y][mask], s=1)

                axarr[1+j].set_xlim(data['load_coords'][xi] - data['radius']*1.5,
                        data['load_coords'][xi] + data['radius']*1.5)
                axarr[1+j].set_ylim(data['load_coords'][yi] - data['radius']*1.5,
